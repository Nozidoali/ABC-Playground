...
if __name__ == '__main__':
    ...
    figure_dir = os.path.join(EXP_HOME, 'figures', 'e1.png')
    data_dir   = os.path.join(EXP_HOME, 'data')

    csv_dir    = os.path.join(data_dir, 'e1.csv')
    skip_data = os.path.exists(csv_dir)

    command_str_all = ['rwz', 'rfz', 'rw', 'rf', 'rs', 'compress2rs', 'rw;rf;rs', 'rf;rw;rs', 'rfz;rwz;rs', 'rwz;rfz;rs']
    if skip_data == False:
        ...
        with open(csv_dir, 'w') as f:
            ...
            for command_str in command_str_all:
                ...
                init_playground(bmark_dir, os.path.join(result_dir, '{}_{}.blif'.format
                        (command_str.replace(';','_'), iteration)))
                ...
                while True:
                    ...
                    output_name = os.path.join(result_dir, '{}_{}.blif'.format
                        (command_str.replace(';','_'), iteration+iteration_step))
